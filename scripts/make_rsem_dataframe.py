...
files = glob.glob("output/*/fastqs/*.genes.results")
line_ct = sum(1 for line in open(files[0]))

ids = [line.split('\t')[0].strip("gene:") for line in open(files[0])]
ids[0] = "gene_id"
isERCC = np.asarray([idd.startswith("ERCC") for idd in ids[1:]])
...
counts = [np.array(ids)]
tpms = [np.array(ids)]
re_plate = re.compile("18_(\\d+)")
for file in files:
    counts.append(np.array([line.split('\t')[4] for line in open(file)]))
    tpms.append(np.array([line.split('\t')[5] for line in open(file)]))
    print("reading "+ re_plate.search(file).groups()[0] + "_" + os.path.basename(file).split(".")[0])

print(f"Saving to {outcounts} ...")
dataframe = np.row_stack(counts)
dataframe[1:,0] = [re_plate.search(file).groups()[0] + "_" + os.path.basename(file).split(".")[0]]
pddf = pd.DataFrame(dataframe[1:,1:], index=dataframe[1:,0], columns=dataframe[0,1:]).sort_index()
pddf[np.array(ids[1:])[~isERCC]].to_csv(outcounts)
pddf[np.array(ids[1:])[isERCC]].to_csv(outcounts + ".ercc.csv")

print(f"Saving to {outtpms} ...")
dataframe = np.row_stack(tpms)
dataframe[1:,0] = [re_plate.search(file).groups()[0] + "_" + os.path.basename(file).split(".")[0] for file in files]
pddf = pd.DataFrame(dataframe[1:,1:], index=dataframe[1:,0], columns=dataframe[0,1:]).sort_index()
pddf[np.array(ids[1:])[~isERCC]].to_csv(outtpms)
pddf[np.array(ids[1:])[isERCC]].to_csv(outtpms + ".ercc.csv")
